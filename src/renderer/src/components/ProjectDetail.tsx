...
const TABS = ['Dashboard', 'Scope', 'Input', 'RFI', 'Queries', 'Dispatch', 'Tasks', 'Status', 'WIP', 'QC', 'Meetings', 'Timesheet', 'Standards', 'Feedback'] as const
type Tab = (typeof TABS)[number]

const COUNT_TYPES: Record<string, string> = {
  Scope: 'scope', Input: 'input', RFI: 'rfi', Queries: 'query', Dispatch: 'dispatch', Tasks: 'task',
  WIP: 'wip', QC: 'qc', Meetings: 'meeting', Timesheet: 'timesheet', Standards: 'standard', Feedback: 'feedback'
}
...
export default function ProjectDetail({ project, onUpdate, onDelete, onToast, onBack, gotoTab, refreshKey = 0 }: Props) {
    ...
    <div className="project-detail">
      <div className="project-header">
        ...
        <div className="project-header-right">
          ...
          <button className="btn btn-secondary btn-sm" onClick={() => setEditing(true)}><Icon name="edit" size={15} /> Edit</button>
      ...
      <div className="tab-host" key={refreshKey}>
        ...
        {activeTab === 'Scope' && <PathTab type="scope" singular="Scope Item" {...tabProps} hint="No scope items yet. Add a title and the file/folder path to your scope documents, then open them from here." />}
        {activeTab === 'Input' && <PathTab type="input" singular="Input" {...tabProps} hint="No inputs yet. Link the file/folder path to received inputs and open them from here." />}
        {activeTab === 'RFI' && <RFITab {...tabProps} />}
        {activeTab === 'Queries' && <QueryTab {...tabProps} />}
