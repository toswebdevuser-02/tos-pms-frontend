...
- **Overtime approvals** (Team Lead+) — two-stage OT approval (see §8).
- **Assign projects** (Team Lead+) — attach members to projects.
- **Discipline roll-up** (Team Lead+) — portfolio grouped by discipline.
- **Executive overview** (Team Lead+) — risk, utilization, budget forecast, email digest.

**Sales & Clients**
- **Quote a project** (Team Lead+) — create/approve quotations (see §6).
- **Client dashboard** — portfolio grouped by client.
- **Fill timesheet (by client)** — lead tool to log time across a client's projects.
