...
export default function CommandPalette({ projects, members, onClose, onNavigate }: Props) {
  ...
  const [items, setItems] = useState<{ tasks: Row[]; rfis: Row[]; queries: Row[] }>({ tasks: [], rfis: [], queries: [] })
  ...
  // Load cross-project items once on open.
  useEffect(() => {
    let alive = true
    ;(async () => {
      const tasks: Row[] = [], rfis: Row[] = [], queries: Row[] = []
      await Promise.all(projects.map(async (p) => {
        const [t, r, qq] = await Promise.all([
          window.api.items.getByProject(p.id, 'task'),
          window.api.items.getByProject(p.id, 'rfi'),
          window.api.items.getByProject(p.id, 'query')
        ])
        if (t.ok) (t.data as Row[]).forEach((x) => tasks.push({ ...x, project_id: p.id }))
      ...
      if (alive) setItems({ tasks, rfis, queries })
    })()
    return () => { alive = false }
  }, [projects])
  ...
  const hits = useMemo<Hit[]>(() => {
    ...
    for (const r of items.rfis) {
      if (match(r.subject, r.rfi_number, r.description)) out.push({ key: `r${r.id}`, icon: 'send', title: s(r.subject) || `RFI ${s(r.rfi_number)}`, sub: `RFI · ${projName.get(Number(r.project_id)) ?? ''}`, group: 'RFIs', target: { kind: 'item', projectId: Number(r.project_id), tab: 'RFI' } })
    }
    for (const r of items.queries) {
      if (match(r.subject, r.query_number, r.description)) out.push({ key: `q${r.id}`, icon: 'help', title: s(r.subject) || `Query ${s(r.query_number)}`, sub: `Query · ${projName.get(Number(r.project_id)) ?? ''}`, group: 'Queries', target: { kind: 'item', projectId: Number(r.project_id), tab: 'Queries' } })
    }
    return out.slice(0, 40)
  }, [q, projects, members, items, projName])
